...
def _dispatch() -> int:
    if len(sys.argv) == 5 and sys.argv[1] == '--packaging-gui-mode-smoke':
        from djd_maker.packaging.limit_dual_smoke import run_gui_mode_smoke
        return run_gui_mode_smoke(Path(sys.argv[2]), Path(sys.argv[3]), sys.argv[4])
    if len(sys.argv) == 4 and sys.argv[1] == '--packaging-limit-dual-smoke':
        from djd_maker.packaging.limit_dual_smoke import run_limit_dual_smoke
        return run_limit_dual_smoke(Path(sys.argv[2]), Path(sys.argv[3]))
