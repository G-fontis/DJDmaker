...
class JobDetailDialog(QDialog):
    ...
    def __init__(self, job: Job, parent: QWidget | None = None) -> None:
        ...
        form.addRow("Remote Source", QLabel(job.source_status))
        form.addRow("Remote Artifact", QLabel(job.artifact_status))
        form.addRow("Local error", QLabel(job.local_error_code or "－"))
        form.addRow("Recovered checkpoint", QLabel(job.remote_checkpoint or "－"))
