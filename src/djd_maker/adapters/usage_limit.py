    ...
    def observe(self):
        values = self.page.evaluate("""() => {
          ...
          for (const e of document.querySelectorAll('body *')) {
            ...
            if (/使用量上限|まで無効|以降にすべての機能|利用可能になります/.test(text)) messages.push(text);
          }
          for (const e of inputs) messages.push(e.getAttribute('placeholder') || '', e.getAttribute('aria-label') || '');
          return {text: messages.join('\\n'), disabled: inputs.some(disabled) || sends.some(disabled), enabled: inputs.length === 1 && !disabled(inputs[0])};
        }""")
