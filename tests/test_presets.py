...
def test_preset_repository_save_load_and_default(tmp_path) -> None:
    ...
    assert PresetRepository(path).selected() == created
    payload = json.loads(path.read_text(encoding="utf-8"))
    assert payload["schema_version"] == SCHEMA_VERSION
    assert payload["kind"] == "presets"


def test_preset_create_edit_select_and_restart_restore(tmp_path) -> None:
    path = tmp_path / "presets.json"
    repository = PresetRepository(path)
    ...
    assert restarted.selected() == changed
...
def test_stale_legacy_lock_does_not_block_preset_save(tmp_path) -> None:
    ...
    assert PresetRepository(path).selected() is not None
