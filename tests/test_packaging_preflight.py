...
def test_complete_release_tree_passes_structural_scan(tmp_path: Path) -> None:
    root = tmp_path / "DJDmaker_Ver1.2.1"
...
def test_spec_is_windowed_onedir_and_build_script_never_creates_zip() -> None:
    ...
    assert 'name="DJDmaker_Ver1.2.1"' in spec
    ...
    assert "ProductVersion', u'1.2.1'" in version_info
    assert "FileVersion', u'1.2.1.0'" in version_info
    assert "filevers=(1, 2, 1, 0)" in version_info
    assert "prodvers=(1, 2, 1, 0)" in version_info
