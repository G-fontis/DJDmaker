...
def test_complete_release_tree_passes_structural_scan(tmp_path: Path) -> None:
    root = tmp_path / "DJDmaker_Ver1.2.7"
...
def test_spec_is_windowed_onedir_and_build_script_never_creates_zip() -> None:
    ...
    assert 'name="DJDmaker_Ver1.2.7"' in spec
    ...
    assert "ProductVersion', u'1.2.7'" in version_info
    assert "FileVersion', u'1.2.7.0'" in version_info
    assert "filevers=(1, 2, 7, 0)" in version_info
    assert "prodvers=(1, 2, 7, 0)" in version_info
...
def test_packaged_row_verifier_uses_current_queued_presentation_contract() -> None:
    source = (PROJECT_ROOT / 'src/djd_maker/packaging/sequential_smoke.py').read_text(encoding='utf-8')
    assert 'window.controller.presentation_event.connect(probe.receive, Qt.ConnectionType.QueuedConnection)' in source
    assert 'event.id != EventId.JOB_UPDATED' in source
    assert "{'chat.send','generation.accepted','hls.start','COMPLETED'}<=refreshed" in source
    assert 'window.controller.job_changed.connect(probe.receive)' not in source
